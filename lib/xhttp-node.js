...
const {isComplex, isFunction, isObject, isString, isBoolean, isFinite,
...
function parseParams(params) {
  validate(params, isObject)
  const {protocol, hostname, port, path, hash, auth} = parseUrl(params.url)
  return patchDicts({protocol, hostname, port, path: `${path}${hash || ''}`, auth}, params, {url: null})
}

exports.bufferBody = bufferBody
function bufferBody(response) {
  validate(response, isResponse)
  return response.stream
    ? bufferStream(response.stream)
      .mapResult(function bufferedResponse(body) {
        return patchDicts(response, {stream: null, body})
      })
    : response.body == null
    ? patchDicts(response, {body: Buffer.alloc(0)})
...
function stringifyBody(response) {
  validate(response, isResponse)
  validate(response.body, isBufferOrString)
  return patchDicts(response, {body: String(response.body)})
}
...
function maybeParseBody(response) {
  ...
  return /application[/]json/.test(contentType)
    ? patchDicts(response, {body: JSON.parse(body)})
    : response
}
...
function bufferStream(stream) {
  ...
  function onData(chunk) {
    try {
      validate(chunk, isBufferOrString)
      chunks.push(chunk)
    }
    catch (err) {
      future.settle(err)
...
function toJsonParams(params) {
  ...
  return patchDicts(params, {
    headers: patchDicts(jsonHeaders, headers),
...
function patchDicts(...values) {
  const out = {}
  for (const value of values) {
    if (!isComplex(value)) continue
