...
function stringifyBody(response) {
  f.validate(response, isResponse)
  f.validate(response.body, isBufferOrString)
...
function concatChunks(chunks) {
  return chunks.some(f.isString) ? chunks.join('') : Buffer.concat(chunks)
}
...
function deinitStream(stream) {
  if (f.isFunction(stream.destroy)) stream.destroy()
  else if (f.isFunction(stream.end)) stream.end()
}
